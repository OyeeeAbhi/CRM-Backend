...
export default class TicketService {
    ...
    async createTicket(ticketDetails: CreateTicketDto, userId: string) : Promise<Ticket> {
        try {
            ...
            this.mailer.sendEmail(ticket.createdBy || '', `Successfully created your ticket ${ticket.id}`, ticketCreated(ticket.id)); // sending email to the user who created the ticket
            this.mailer.sendEmail(ticket.assignedTo || '' , `You have been assigned a ticket ${ticket.id}`, ticketCreated(ticket.id)); // sending email to the engineer who is assigned with ticket
            return ticket;
        } catch(error) {
            if(error instanceof BadRequestError) {
                throw error;
